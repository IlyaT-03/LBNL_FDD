...
from lbnl_fdd.models.lstm import LSTMClassifier
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train LSTMClassifier")

    parser.add_argument("--dataset", type=str, required=True)
    parser.add_argument("--data_root", type=str, default="data/processed")
    parser.add_argument("--save_root", type=str, default="outputs/runs")
    parser.add_argument("--run_name", type=str, default="lstm_run1")
...
def evaluate_and_save(model, dataset, batch_size, device, average, save_dir, split_name):
    metrics, y_true, y_pred = eval_windows_accuracy_f1_timesnet(
        model=model, window_ds=dataset,
        batch_size=batch_size, device=device, average=average,
    )
...
def main():
    ...
    train_ds = build_dataset(train_df, train_target, args.window_size, args.stride,
                             args.train_windows_file, "train")
    val_ds   = build_dataset(val_df,   val_target,   args.window_size, args.stride,
                             args.val_windows_file,   "val")
    test_ds  = build_dataset(test_df,  test_target,  args.window_size, args.stride,
                             args.test_windows_file,  "test")
    ...
    if args.eval_test:
        ...
        evaluate_and_save(model, test_ds, args.batch_size, device,
                          args.average, save_dir, "test_last_epoch")
    ...
    if args.eval_train:
        evaluate_and_save(model, train_ds, args.batch_size, device,
                          args.average, save_dir, "train")
    if args.eval_test:
        print("Evaluating test set using best model...")
        evaluate_and_save(model, test_ds, args.batch_size, device,
                          args.average, save_dir, "test")
