...
def train_gnn(
    ...
    """
    Minimal training loop for current GNN_TAM + SlidingWindowDataset setup.

    Expected dataset output:
        x: (window_size, n_features)
        y: scalar class label

    Model input after transpose:
        x: (B, n_features, window_size)
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x, y in progress:
            x = x.to(device).transpose(1, 2)  # (B, T, F) -> (B, F, T)
            y = y.to(device).long()
        ...
        if val_ds is not None:
            val_metrics, _, _ = eval_windows_accuracy_f1(
                model=model,
                window_ds=val_ds,
                batch_size=batch_size,
                device=device,
